  ...
  <el-container style="height: 100vh">
    <el-aside width="14vw" style="background-color: var(--nav--color)">
      ...
      <div class="add-container">
        <button class="add" @click="createDoc()">
          <i class="ri-add-line"></i>&nbsp;新建文档
        </button>
        <button class="import" @click="importDoc()">
          <i class="ri-import-line"></i>&nbsp;导入文档
        </button>
        <input
          type="file"
          ref="fileInput"
      ...
      <div class="recent-access">
        ...
        <ul class="recent-list" v-if="displayedRecentDocs.length > 0">
          <li
            v-for="(doc, index) in displayedRecentDocs"
            :key="index"
          ...
          <li
            v-if="recentAccessDocs.length > 3 && !showAllRecent"
            class="show-more"
            @click.stop="toggleShowAllRecent"
          >
            <i class="ri-more-line"></i>&nbsp;查看更多
          </li>
          <li
            v-if="showAllRecent && recentAccessDocs.length > 3"
            class="show-more"
            @click.stop="toggleShowAllRecent"
          >
  ...
  try {
    if (typeof knowledgeBaseStore.recordDocumentAccess === "function") {
      ...
      return knowledgeBaseStore
        .recordDocumentAccess(docId, title)
        .catch((err) => {
