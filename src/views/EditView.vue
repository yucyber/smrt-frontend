...
const loadDocuments = async () => {
  try {
    ...
    if (isNewDoc) {
      ...
      // 为新文档设置默认内容和标题
      editor.value.commands.setContent('<h1>未命名文档</h1><p>开始输入文档内容...</p>');
      title.value = "未命名文档";
    ...
    if (response.code == 200) {
      ...
      // 设置默认内容
      editor.value.commands.setContent('<h1>文档加载失败</h1><p>无法加载文档内容，请检查网络连接或刷新页面重试。</p>');
      title.value = "文档加载失败";
    ...
    // 设置默认内容
    editor.value.commands.setContent('<h1>文档加载失败</h1><p>发生错误: ' + (error.message || '未知错误') + '</p>');
    title.value = "文档加载失败";
  } finally {
    NProgress.done();
  }
