<template>
  <div class="knowledge-base-container">
    <div class="page-header">
      <h1>
        <template v-if="knowledgeBaseId">{{
          currentKnowledgeBase?.name || "知识库详情"
        }}</template>
            ...
            <el-table-column fixed="right" label="操作" width="120">
              <template #default="scope">
                ...
                <el-button
                  ...
                  type="danger"
                  size="small"
                  @click="removeDocumentFromKnowledgeBase(scope.row.id)"
                  >移除</el-button
                >
              </template>
            </el-table-column>
          ...
          <el-form :model="knowledgeBaseForm" label-width="120px">
            ...
            <el-form-item>
              <el-button type="primary" @click="saveKnowledgeBase"
                >保存</el-button
              >
              <el-button type="danger" @click="confirmDeleteKnowledgeBase"
                >删除知识库</el-button
              >
            </el-form-item>
          </el-form>
    ...
    >
      <div
        v-if="filteredDocumentsForDialog.length === 0"
        class="empty-docs-dialog"
      >
        <el-empty description="没有可添加的文档" />
      </div>
      <el-table
        v-else
        :data="filteredDocumentsForDialog"
        style="width: 100%"
        height="350px"
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="addDocumentsToKnowledgeBase"
            >确认</el-button
          >
        </span>
      </template>
    ...
    <el-dialog title="确认删除" v-model="showDeleteConfirmDialog" width="30%">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="danger" @click="deleteKnowledgeBase"
            >确认删除</el-button
          >
        </span>
      </template>
    ...
    <el-dialog title="创建知识库" v-model="showCreateDialog" width="500px">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="submitCreateKnowledgeBase"
            >创建</el-button
          >
        </span>
      </template>
    ...
    if (response && response.code === "200") {
      ...
      allDocuments.value = allDocuments.value.map((doc) => {
        ...
        const title =
          doc.title ||
          (doc.content
            ? doc.content.replace(/<[^>]*>/g, "").substring(0, 20) + "..."
            : "未命名文档");
        return {
          ...doc,
          title: title,
          updateTime:
            doc.updated_at || doc.updateTime || new Date().toLocaleString(),
        };
      });
...
const addDocumentsToKnowledgeBase = async () => {
  ...
  try {
    ...
    for (const docId of docIds) {
      const success = await knowledgeBaseStore.addDocumentToKnowledgeBase(
...
const removeDocumentFromKnowledgeBase = async (docId) => {
  try {
    ...
    // 调用store中的方法移除文档
    const success = await knowledgeBaseStore.removeDocumentFromKnowledgeBase(
      knowledgeBaseId.value,
      docId
    );
    ...
    if (success) {
      ...
      // 刷新知识库文档关联
      await knowledgeBaseStore.refreshDocumentAssociations(
        knowledgeBaseId.value
      );
...
const saveKnowledgeBase = async () => {
  ...
  try {
    ...
    if (success) {
      // 更新当前知识库显示
      currentKnowledgeBase.value = knowledgeBaseStore.getKnowledgeBaseById(
        knowledgeBaseId.value
      );
...
const confirmDeleteKnowledgeBase = () => {
  ElMessageBox.confirm("确定要删除该知识库吗？此操作无法撤销。", "删除确认", {
...
const deleteKnowledgeBase = async () => {
  loading.value = true;
  try {
    const success = await knowledgeBaseStore.deleteKnowledgeBase(
      knowledgeBaseId.value
    );
...
const getDocumentCount = (knowledgeBaseId) => {
  try {
    ...
    if (documentCountCache.value[kbIdStr] === undefined) {
      ...
      const count =
        knowledgeBaseStore.getDocumentsInKnowledgeBase(kbIdStr).length || 0;
